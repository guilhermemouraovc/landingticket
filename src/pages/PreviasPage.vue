<template>
  <q-page class="bg-landing">
    <div class="list-wrap">
      ...
      <PageHeader title="Prévias de Carnaval" />
      ...
      <EmptyState
        ...
        title="Nenhuma prévia de Carnaval encontrada"
        message="Não encontramos prévias de Carnaval no momento. Volte mais tarde!"
        icon="celebration"
      ...
      <!-- Grid de eventos -->
      <div v-else class="cards-grid" role="list" aria-label="Lista de prévias de Carnaval">
        <EventCard
          v-for="card in items"
          :key="card.id"
          :event="card"
...
const breadcrumbItems = computed(() => [
  ...
  { label: 'Prévias de Carnaval', to: null },
])

onMounted(async () => {
  await loadCategories()
  await loadPreviasEvents()
})
...
async function loadPreviasEvents() {
  loading.value = true
  try {
    ...
    if (!events.length) {
      events = await fetchEventsByTag('Prévias de Carnaval', { limit: 100 })
    }

    if (!events.length) {
      events = await fetchEventsByTag('previas-carnaval', { limit: 100 })
    }

    items.value = events
  } catch (e) {
    if (import.meta.env.DEV) {
      console.error('❌ Falha ao carregar prévias de Carnaval', e)
    }
    items.value = []
  } finally {
    loading.value = false
