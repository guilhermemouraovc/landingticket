            ...
            <div class="featured-grid rounded-borders overflow-hidden shadow-2">
              ...
              <div class="featured-panel">
                <div class="panel-inner">
                  ...
                  <div
                    v-if="ev.description"
                    class="featured-description text-body1 q-mt-xs"
                  >
                    {{ ev.description }}
                  </div>
    ...
    <footer class="footer">
      <div class="footer-wrap">
        ...
        <div class="text-center q-mt-md">
          © 2025 Ticketpe. Todos os direitos reservados.
        </div>
      </div>
    </footer>
...
async function loadFeatured () {
  try {
    ...
    const festas = Array.isArray(response?.data?.data)
      ? response.data.data
      : []
...
function createPlaceholderCards (prefix) {
  const baseSlug = normalizeForSearch(prefix)
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '') || 'evento'
...
.featured-panel { background: #fff; color: #1f2937; display: flex; }
